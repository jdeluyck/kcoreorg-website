...
The policies available on your system can be found at `/usr/share/crypto-policies`{: .filepath}. The default is aptly named [`DEFAULT`](https://gitlab.com/redhat-crypto/fedora-crypto-policies/-/blob/master/policies/DEFAULT.pol), but to be able to connect to a Unifi device, you'll need to switch back to [`LEGACY`](https://gitlab.com/redhat-crypto/fedora-crypto-policies/-/blob/master/policies/LEGACY.pol) to be able to connect.
