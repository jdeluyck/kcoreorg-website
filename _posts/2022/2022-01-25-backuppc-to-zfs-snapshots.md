...
The tool is really simple: you add the backup policy to `/etc/sanoid/sanoid.conf`{: .filepath}, specifying which snapshots to take and how to lifecycle them. On Debian a [systemd timer](https://www.freedesktop.org/software/systemd/man/systemd.timer.html) comes with the package that executes `sanoid` every 15 minutes.
