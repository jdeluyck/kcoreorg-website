...
Setting `net.ipv6.conf.eth0.accept_ra=2` in `/etc/sysctl.d/99-docker.conf`{: .filepath} didn't help much: it seems that Docker will overwrite this setting too. So I opted instead to create a new [systemd](https://systemd.io/) service file called `/etc/systemd/system/docker-ipv6-route.service`{: .filepath} to reset this value:
