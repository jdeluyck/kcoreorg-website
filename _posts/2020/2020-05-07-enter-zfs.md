...
Quoting the [FreeNAS](https://www.truenas.com/) website:
...
This seemed like a good solution for the NAS part of the equation. It's built on [FreeBSD](https://www.freebsd.org/),
offers [ZFS](https://docs.freebsd.org/en/books/handbook/zfs/) for storage, a bunch of [plugins](http://web.archive.org/web/20200506085629/https://www.freenas.org/plugins/)[^ia1]
...
and [tests](https://www.truenas.com/community/threads/virtual-machine-performance-11-2-u3.76224/)
...
So, to solve the virtualisation issue, there was the halfbaked idea of running [VMWare](https://www.vmware.com/)'s
[ESXi](http://web.archive.org/web/20200811110426/https://www.vmware.com/be/products/esxi-and-esx.html)[^ia2] as a hypervisor, run FreeNAS as a VM and share the storage
...
[sync](https://www.truenas.com/community/threads/sync-writes-or-why-is-my-esxi-nfs-so-slow-and-why-is-iscsi-faster.12506/) mode.  
Sync mode is slow (it needs to actively sync every change to disk), so you need an additional [SLOG](https://www.truenas.com/community/threads/some-insights-into-slog-zil-with-zfs-on-freenas.13633/) device.  
The SLOG device also needs to have [power loss protection](https://www.truenas.com/community/threads/list-of-ssds-with-power-loss-protection.63998/), because you *need* to be sure the writes are consistent.
...
From the [Proxmox](https://proxmox.com/en/products/proxmox-virtual-environment/overview) website:
...
This is basically [Debian](https://www.debian.org/), with a hypervisor GUI, running on top of ZFS. It doesn't offer
anything of the other things, but it being Debian, it's not that hard to make it do all the things that I want it to do.

* NAS functionality will be done with ZFS and sharing through [Samba](https://www.samba.org/) and [NFS](https://en.wikipedia.org/wiki/Network_File_System).  
...
* [Sharkoon SilentStorm SFX 500 Gold](https://en.sharkoon.com/product/SilStorSFXGold) PSU
...
* [Cooler Master Hyper 212 EVO](https://www.coolermaster.com/en-global/products/hyper-212-evo/) CPU cooler
...
* 4 [WD Red](https://www.westerndigital.com/en-ie/products/internal-drives/wd-red-sata-hdd) 6TB [PMR](https://en.wikipedia.org/wiki/Perpendicular_recording) drives
...
* Defining new [storage endpoints](https://pve.proxmox.com/wiki/Storage) in `/etc/pve/storage.cfg`{: .filepath} for backups and images
...
* [Unifi Network Management Controller](https://www.ui.com/download/unifi/) (on a different VLAN)
* General purpose container (for some tools)

### Virtual Machines

At the moment only one VM is running, which hosts my [WireGuard](https://www.wireguard.com/) endpoint. Reason why this runs
in a VM is because it requires kernel modules, and I don't want to add anything extra to my main Proxmox installation.

### Docker configuration

Docker has been configured to use the [zfs storage driver](https://docs.docker.com/engine/storage/drivers/zfs-driver/),
...
to `/etc/docker/daemon.json`{: .filepath}.

For networking I'm using both a user-defined [bridge](https://docs.docker.com/engine/network/drivers/bridge/), which has several
advantages over using the default bridge (automatic DNS resolution between containers for one), and
[macvlan](https://docs.docker.com/engine/network/drivers/macvlan/) to assign external (from Docker's point of view) IP's to docker
containers.
...
* [Watchtower](https://containrrr.dev/watchtower/) to keep containers automatically up-to-date ([dockerhub](https://hub.docker.com/r/v2tec/watchtower))
...
* [Nextcloud](https://nextcloud.com/) for internal file sharing and collaboration ([dockerhub](https://hub.docker.com/_/nextcloud/))
...
* [Plex](https://watch.plex.tv/) for streaming to devices ([dockerhub](https://hub.docker.com/r/plexinc/pms-docker))
* [Portainer](https://www.portainer.io/) for easy Docker Container management ([dockerhub](https://hub.docker.com/r/portainer/portainer/))
* [Grafana](https://grafana.com) for fancy dashboards and graphs ([dockerhub](https://hub.docker.com/r/grafana/grafana))
* [InfluxDB](https://www.influxdata.com/) as a datastore for Grafana (for data coming from Home Assistant) ([dockerhub](https://hub.docker.com/_/influxdb))
...
[^ia1]: Internet Archive snapshot. Original URL: https://www.freenas.org/plugins/ <!-- markdownlint-disable-line MD034 -->

[^ia2]: Internet Archive snapshot. Original URL: https://www.vmware.com/be/products/esxi-and-esx.html <!-- markdownlint-disable-line MD034 -->
